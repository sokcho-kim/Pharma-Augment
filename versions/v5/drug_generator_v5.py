...
4) 질문 유형을 다양하게 (V2 스타일):
   A) 기본 정보형: "{main_name}의 구체적인 급여 인정 기준은 무엇인가요?"
   B) 조건/상황형: "특정수치/조건일 때 {main_name} 사용이 가능한가요?"
   C) 비교형: "{main_name}의 경구제와 주사제 급여 기준 차이점은?"
   D) 절차형: "{main_name} 처방 시 필요한 사전승인 절차는 어떻게 되나요?"
   E) 제한형: "어떤 경우에 {main_name} 사용이 제한되거나 삭감되나요?"
